...
class FormModel
{
    ...
    {
        foreach ($fields as $name) $this->field($name);
    }
        ...
        foreach ($this->fields as $name => $field)
            if ($field instanceof FormAttribute) {
                ...
                if ($field instanceof FormFile) {
                    if (!is_null($value)) $this->files[$name] = new FileUpload($value);
                } else $this->parsedBody[$name] = $value;
    ...
    {
        foreach ($this->js as $script) echo '<script src="/resources/js/' . $script . '"></script>';
        echo '</form>';
    }
    ...
    public function getSuccess(): void
    {
        if (strlen($this->success) > 0)
            echo <<<HTML
                <div class="w-full items-center my-1 p-4 bg-green-600/[.2] text-green-600/[.9] rounded-xl flex my-4">
