...
class SoutenanceRepository extends AbstractRepository
{
	...
	{
		$result = self::Fetch("SELECT * FROM " . self::$table . " WHERE idetudiant = :id AND numconvention = :numconvention", [
			'id' => $id,
	...
	{
		$result = self::Fetch("SELECT * FROM " . self::$table . " WHERE idtuteurentreprise = :id AND numconvention = :numconvention", [
			'id' => $id,
			'numconvention' => $getNumConvention
		]);
		return $result != null;
	...
	{
		$result = self::Fetch("SELECT * FROM " . self::$table . " WHERE idtuteurprof = :id AND numconvention = :numconvention", [
			'id' => $id,
			'numconvention' => $getNumConvention
		]);
		return $result != null;
	...
	{
		$result = self::Fetch("SELECT * FROM " . self::$table . " WHERE idprof = :id AND numconvention = :numconvention", [
			'id' => $id,
			'numconvention' => $getNumConvention
		]);
		return $result != null;
	...
	public static function createSoutenance(?array $array): void
	{
		...
		self::Execute("INSERT INTO soutenances(numconvention, idtuteurprof, idtuteurentreprise, debut_soutenance, fin_soutenance) VALUES (:numconvention, :idtuteurprof, :idtuteurentreprise, :debut_soutenance, :fin_soutenance)", [
	...
	{
		$result = self::Fetch("SELECT idsoutenance FROM " . self::$table . " WHERE numconvention = :numconvention", [
			'numconvention' => $numConvention
		]);
		return $result != null;
	}
	...
	{
		self::Execute("UPDATE soutenances SET idprof = :idprof WHERE numconvention = :numconvention", [
			'idprof' => $idprof,
			'numconvention' => $numConvention
		]);
	}
	...
	{
		$result = self::Fetch("SELECT idsoutenance FROM " . self::$table . " WHERE idprof = :idprof AND numconvention = :numconvention", [
			'idprof' => $idprof,
			'numconvention' => $numConvention
		]);
		return $result != null;
	...
	{
		$result = self::Fetch("SELECT idsoutenance FROM " . self::$table . " WHERE idtuteurprof = :idprof AND numconvention = :numconvention", [
			'idprof' => $idprof,
			'numconvention' => $numConvention
		]);
		return $result != null;
