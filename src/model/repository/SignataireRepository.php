...
class SignataireRepository extends AbstractRepository
{
	...
	{
		$result = self::FetchAllAssoc("SELECT * FROM Signataire WHERE idEntreprise = :idEntreprise", [":idEntreprise" => $identreprise]);
		$signataires = [];
	...
	{
		$result = self::FetchAllAssoc("SELECT * FROM Signataire WHERE idEntreprise = :idEntreprise AND nomSignataire= :nomsignataire", [":idEntreprise" => $idEntreprise, ":nomsignataire" => $nomsignataire]);
	...
	public function create(mixed $nomSignataire, mixed $prenomSignataire, mixed $fonctionSignataire, mixed $mailSignataire, mixed $idEntreprise): void
	{
		self::Execute("SELECT creerSignataire(:nomSignataire, :prenomSignataire, :mailSignataire, :fonctionSignataire, :idEntreprise)", [
			":nomSignataire" => $nomSignataire, ":prenomSignataire" => $prenomSignataire, ":fonctionSignataire" => $fonctionSignataire, ":mailSignataire" => $mailSignataire, ":idEntreprise" => $idEntreprise]);
	}
