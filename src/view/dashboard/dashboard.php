 ...
 */
//print "<pre>";
//print_r($statsDensembleStageEtAlternance);
//print "</pre>";
...
$maxOffers = max(array_column($statsDistributionDomaine, 'nombreoffres'));

$totalOffers = array_sum(array_column($statsDistributionDomaine, 'nombreoffres'));
$offset = 0; // This is the starting point for our next "slice"

function createPieChart($stats, $total) {
    $gradientPieces = [];
    $offset = 0;
    ...
    foreach ($stats as $row) {
        ...
        // Determine the colors for each slice - this is something you'd customize.
        //random gray color generator (percentage of black)
        $color = "hsl(0, 0%, " . (95 - $percentage) . "%)";
...
$pieChartStyle = createPieChart($statsDistributionDomaine, $totalOffers);
...
<div class="w-full flex md:flex-row flex-col justify-between items-start gap-4 mx-auto pt-12 pb-24">
    ...
    <div class="relative grow isolate overflow-hidden w-full bg-zinc-50 h-[600px] border rounded-2xl text-[#1A2421] backdrop-blur-xl [ p-8 md:p-10 lg:p-10 ] [ border-[1px] border-solid border-black  border-opacity-10 ] [ shadow-black/5 shadow-2xl ]"></div>
</div>